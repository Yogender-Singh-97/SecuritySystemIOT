...
const user_dashboard_get = async function (req, res) {
  ...
  const users_active = await db.users.count({
    where: {
      status: 'current'
    }
  });
...
const delete_user = async function (req, res) {
  ...
  var errors = [];
  try {
    const users_exist = await db.guard_clocks.count({
      where: {
        user_id: uid
      }
    });
    ...
    if (users_exist > 0) {
      req.flash('alerte1', 'Delete Failed!, User has active records and allocations.');
      errors.push({ msg: 'Delete Failed!, User has active records and allocations.' });
    }
...
//Getting the form for User Update
...
//Saving the Updated Data of User
...
function user_router(req, res) {
  req.user.then(function (result) {
    ...
    if (result.role == 'admin') {
      ...
      res.redirect('/supervisor/sdashboard');
...
const user_change_password = async function (req, res) {
  ...
  var success = [];
  try {
    const password_check = await db.users.findOne({
      where: {
    ...
    if (errors.length < 1) {
      ...
      //saving changed password to database
      await db.users.update({
        password: hash
      }, {
        where: {
...
module.exports = { 
  user_enable, user_disable, user_password_reset, user_change_password_get, user_change_password, user_dashboard_get, 
  user_router, user_register_get, save, manage, delete_user, update_user1, update_user2, login_handler, logout_handler, 
  isAuthenticated, isAuthenticated_role
};